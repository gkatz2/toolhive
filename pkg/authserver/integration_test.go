...
func TestIntegration_ClientBinding_CodeTheftPrevention(t *testing.T) {
	...
	// Attacker tries to exchange the code with their client ID
	attackerParams := url.Values{
		"grant_type":    {"authorization_code"},
		"code":          {authCode},
		"client_id":     {attackerClientID}, // Attacker's client
		"redirect_uri":  {attackerRedirectURI},
		"code_verifier": {verifier},
	}

	resp := makeTokenRequest(t, ts.Server.URL, attackerParams)
	defer resp.Body.Close()

	// Must reject: client binding violation
	require.GreaterOrEqual(t, resp.StatusCode, 400, "should reject code theft attempt")

	errResp := parseTokenResponse(t, resp)
	errorField, ok := errResp["error"].(string)
	require.True(t, ok, "error should be present")
	assert.Contains(t, errorField, "invalid", "should return invalid_grant or invalid_client")
}
...
func TestIntegration_RefreshToken_ClientBinding(t *testing.T) {
	...
	params := url.Values{
		"grant_type":    {"authorization_code"},
		"code":          {authCode},
		"client_id":     {testClientID}, // Legitimate client
		"redirect_uri":  {testRedirectURI},
		"code_verifier": {verifier},
	}
	...
	// Attacker tries to use the refresh token with their client ID
	attackerParams := url.Values{
		"grant_type":    {"refresh_token"},
		"refresh_token": {refreshToken},
		"client_id":     {attackerClientID}, // Attacker's client - not the one that got the token
	}

	attackerResp := makeTokenRequest(t, ts.Server.URL, attackerParams)
	defer attackerResp.Body.Close()

	// Must reject: refresh token client binding violation (RFC 6749 Section 10.4)
	require.GreaterOrEqual(t, attackerResp.StatusCode, 400, "should reject refresh token theft")

	errResp := parseTokenResponse(t, attackerResp)
	errorField, ok := errResp["error"].(string)
	require.True(t, ok, "error should be present")
	assert.NotEmpty(t, errorField, "error field should contain the error type")
}
